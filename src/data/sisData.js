...
const GetAvatar = (name) => {
  ...
  return <img src={photo} className="img-fluid img-radius wid-40" alt={name} />;
};
...
const newPerson = () => {
  ...
  return {
    ...
    status: (
      <React.Fragment>
        {status === 1 && <span className="badge bg-primary inline-block mr-1">Granted</span>}
        {status === 2 && <span className="badge bg-warning inline-block mr-1">Cancelled</span>}
        {status === 3 && <span className="badge bg-success inline-block mr-1">Failed</span>}
