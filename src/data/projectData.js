  ...
  return (
    <React.Fragment>
      <div>
        {count < 2 ? (
          <Link to="#">
            <img className="img-fluid img-radius m-r-5" src={photo} style={{ width: '30px' }} alt="Task List" />
          </Link>
        ) : (
          ''
        )}
        {count < 3 ? (
          <Link to="#">
            <img className="img-fluid img-radius m-r-5" src={photo1} style={{ width: '30px' }} alt="Task List" />
          </Link>
        ) : (
          ''
        )}
        {count === 3 ? (
          <Link to="#">
            <img className="img-fluid img-radius m-r-5" src={photo2} style={{ width: '30px' }} alt="Task List" />
          </Link>
        ) : (
          ''
        )}
        <Link to="#">
          <i className="fas fa-plus f-w-600 mr-5" />
        </Link>
      </div>
    </React.Fragment>
  );
...
const newPerson = () => {
  ...
  return {
    ...
    group: (
      <React.Fragment>
        {Math.floor(Math.random() * 2 + 1) > 1 && <span className="badge bg-danger inline-block mr-1">Low Budget</span>}
        {Math.floor(Math.random() * 2 + 1) > 1 && <span className="badge bg-success inline-block mr-1">High Budget</span>}
        {Math.floor(Math.random() * 2 + 1) > 1 && <span className="badge bg-warning inline-block mr-1">VIP</span>}
        {Math.floor(Math.random() * 2 + 1) > 1 && <span className="badge bg-primary inline-block">Wholesaler</span>}
      </React.Fragment>
    ),
    tags: (
      <React.Fragment>
        {tags === 1 && <span className="badge bg-danger inline-block mr-1">Wordpress</span>}
        {tags === 2 && <span className="badge bg-success inline-block mr-1">Vue</span>}
        {tags === 3 && <span className="badge bg-warning inline-block mr-1">React</span>}
    ...
    status: (
      <React.Fragment>
        {status === 1 && <span className="badge bg-primary inline-block mr-1">In Proccess</span>}
        {status === 2 && <span className="badge bg-warning inline-block mr-1">Delay</span>}
        {status === 3 && <span className="badge bg-success inline-block mr-1">Completed</span>}
